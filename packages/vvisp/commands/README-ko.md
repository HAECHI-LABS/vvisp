...
> - `scripts/test.sh`가 생성되고 `$ npm consoleTest test` 스크립트가 `package.json`에 추가됩니다. truffle의 contract testing을 할 때 유용합니다.
> - `scripts/coverage.sh`가 생성되고 ` $ npm consoleTest coverage` 스크립트가 `package.json`에 추가됩니다. 컨트랙트 테스트 코드의 커버리지를 보여줍니다.
