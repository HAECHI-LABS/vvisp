...
describe('# deploy contract process test', function() {
  ...
  it('should deploy one contract', async () => {
    await deployContract(CONTRACT_PATH, [], testOption).should.be.fulfilled;
  });
  ...
  it('should reject dummy file', async () => {
    await deployContract(DUMMMY_FILE, [], testOption).should.be.rejected;
  });

  it('should reject when contract name and file name are different', async () => {
    await deployContract(CONTRACT_FLATTENED_PATH, [], testOption).should.be
      .rejected;
  });

  it('should deploy inherited file', async () => {
    await deployContract(CONTRACT_INHERITED_PATH, ['0x0'], testOption).should.be
      .fulfilled;
  });
});
